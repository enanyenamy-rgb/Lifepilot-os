    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  isArabic ? 'نظام تشغيل متكامل ينبض بالحياة، يتفاعل مع كل سؤال بشكل مخصص وفوري.' : 'An interactive OS responding dynamically to every query.',
          ...
          _buildFeatureBadge(context, Icons.local_hospital, Colors.red, isArabic ? 'القسم الطبي والتخصصي الفوري' : 'Medical & Health Hub', const DynamicChatScreen(hubTitle: 'القسم الطبي والتخصصي', hintText: 'اكتب سؤالك الطبي أو الأعراض...')),
          _buildFeatureBadge(context, Icons.chair, Colors.brown, isArabic ? 'هندسة الديكور وتصميم المنازل' : 'Interior Architecture Hub', const DynamicChatScreen(hubTitle: 'هندسة الديكور وتصميم المنازل', hintText: 'اكتب المساحة، الغرفة، والنمط (مودرن/كلاسيك)...')),
          _buildFeatureBadge(context, Icons.checkroom, Colors.purple, isArabic ? 'مستشار الموضة وتنسيق الأزياء' : 'Fashion & Style Hub', const DynamicChatScreen(hubTitle: 'مستشار الموضة والأزياء', hintText: 'اختر الفئة والمناسبة (رجال، نساء، أطفال)...')),
          _buildFeatureBadge(context, Icons.auto_awesome, Colors.amber, isArabic ? 'المساعد المعرفي الشامل' : 'General Knowledge Hub', const DynamicChatScreen(hubTitle: 'المساعد المعرفي العام', hintText: 'اطرح أي سؤال تريد معرفته...')),
        ],
      ),
    );
  }
...
class _DynamicChatScreenState extends State<DynamicChatScreen> {
  ...
  void _sendMessage() {
    ...
    // توليد رد ذكي مخصص بناءً على نص السؤال الحالي الذي كتبه المستخدم
    Future.delayed(const Duration(milliseconds: 600), () {
      String aiResponse = '';
      if (widget.hubTitle.contains('الطب')) {
        aiResponse = '🩺 تحليل استفسارك الطبي حول ("$userQuery"):\n\n'
            '• التشخيص المبدئي: بناءً على ما ذكرته، قد يتعلق الأمر بإجهاد عام أو أعراض مرتبطة بالحالة.\n'
            '• خطة التعامل والراحة: يُنصح بشرب السوائل الدافئة، أخذ قسط كافٍ من الراحة، ومتابعة المؤشرات الحيوية.\n'
            '• تنبيه هامة: هذه إجابة توجيهية ذكية، ويُفضل دائماً مراجعة الطبيب المختص للتشخيص الدقيق.';
      } else if (widget.hubTitle.contains('الديكور')) {
        aiResponse = '🏠 استجابة لطلبك التصميمي ("$userQuery"):\n\n'
            '• التخطيط الهندسي: تم بناء التخيل بناءً على المساحة والطلب لضمان الاستغلال الأمثل وتوفير مساحات حركة سلسة.\n'
            '• تنسيق الألوان والأثاث: تم دمج درجات الألوان المتناسقة مع أثاث عصري مريح وإضاءة ليد مخفية تضفي فخامة على المكان.';
      } else if (widget.hubTitle.contains('الموضة')) {
        aiResponse = '👗 استجابة لطلبك في الموضة والأزياء ("$userQuery"):\n\n'
            '• تنسيق الألوان والإطلالة: بناءً على المناسبة والفئة المطلوبة، نقترح الألوان الأكثر تناسقاً ورواجاً هذا الموسم.\n'
            '• النصيحة التنسيقية: دمج القطع الكلاسيكية والعصرية يمنحك حضوراً أنيقاً ومميزاً.';
      } else {
        aiResponse = '💡 إجابة مخصصة حول سؤالك ("$userQuery"):\n\n'
            'بناءً على تحليلك ومعطيات السؤال، إليك التفاصيل الشاملة والحلول المقترحة بدقة عالية لتلبية هدفك.';
      }
      ...
      body: ListView(
        ...
        children: [
          ...
          const ListTile(
            ...
            subtitle: Text('LifePilot OS - Dynamic Custom Reply v3.3'),
          ),
        ],
      ),
    );
