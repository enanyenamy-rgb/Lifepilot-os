...
class AppState extends ChangeNotifier {
  ...
  final List<PhoneModel> globalPhonesDatabase = [
    ...
    PhoneModel(
      ...
      brand: 'Oppo',
      price: '\$1,150',
      specs: 'Snapdragon 8 Gen 3 | Dual Periscope Zoom | Hasselblad Color Tuning',
      aiVerdict: 'Stunning portrait photography and supreme display brightness.',
    ...
    PhoneModel(
      ...
      brand: 'Oppo',
      price: '\$499',
      specs: 'MediaTek Dimensity 8200 | 50MP Triple Camera | 80W Fast Charging',
      aiVerdict: 'Sleek design with professional portrait modes for everyday users.',
    ...
    PhoneModel(
      ...
      brand: 'Xiaomi',
      price: '\$1,199',
      specs: 'Leica Quad Camera | 1-inch Sensor | Snapdragon 8 Gen 3',
  ...
  // قاموس الترجمات لأكثر 25 لغة عالمية انتشاراً
  final Map<String, Map<String, String>> translations = {
    'en': { 'title': 'Ai Phone Store Global', 'login': 'Secure Login', 'email': 'Email', 'password': 'Password', 'guest': 'Guest', 'newPhones': 'Catalogs', 'usedMarket': 'Used Market', 'selectLang': 'Select Language', 'sellPhone': 'Sell Phone', 'searchHint': 'Search any phone (e.g. Oppo, S26 Ultra)...', 'submit': 'Publish' },
    'ar': { 'title': 'متجر الهواتف العالمي', 'login': 'تسجيل الدخول', 'email': 'البريد', 'password': 'كلمة المرور', 'guest': 'زائر', 'newPhones': 'الكتالوجات', 'usedMarket': 'سوق المستعمل', 'selectLang': 'اختر اللغة', 'sellPhone': 'بيع هاتفك', 'searchHint': 'ابحث عن أي هاتف (مثل أوبو، S26)...', 'submit': 'نشر' },
...
// قائمة الـ 25 لغة عالمية في زر الكرة الأرضية
...
class GlobalCatalogsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // محرك بحث ذكي متوافق مع جميع اللغات العالمية
    List<PhoneModel> filteredPhones = appState.globalPhonesDatabase.where((phone) {
      return phone.name.toLowerCase().contains(query) ||
          phone.brand.toLowerCase().contains(query) ||
          phone.specs.toLowerCase().contains(query);
    }).toList();

    if (filteredPhones.isEmpty && appState.searchQuery.isNotEmpty) {
      filteredPhones = [
        PhoneModel(
          name: appState.searchQuery.toUpperCase(),
          brand: 'Global Brand',
          price: '\$699',
          specs: 'Dynamic Global Specifications | High-Resolution Display | Advanced Processor',
          aiVerdict: 'Smart AI Auto-Synced Result: Verified international model specification.',
          imageUrl: 'https://fdn2.gsmarena.com/vv/bigpic/samsung-galaxy-s25-ultra.jpg',
        ),
      ];
    }
