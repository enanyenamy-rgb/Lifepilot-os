...
void main() async {
  ...
  // تهيئة الـ Firebase للاتصال بقاعدة البيانات السحابية
  try {
...
class AppState extends ChangeNotifier {
  ...
  List<UsedPhoneItem> userListings = [
    UsedPhoneItem(phoneName: 'iPhone 15 Pro', price: '\$750', location: 'Global Cloud Verified', details: 'Mint condition, 256GB, battery health 90%'),
    UsedPhoneItem(phoneName: 'Samsung S24 Ultra', price: '\$850', location: 'Global Cloud Verified', details: 'Like new with original box and international warranty'),
  ];
  ...
  void addListing(String name, String price, String details) {
    userListings.add(UsedPhoneItem(phoneName: name, price: price, location: 'Global Cloud Verified', details: details));
    notifyListeners();
  }

  final Map<String, Map<String, String>> translations = {
    'en': {
      'title': 'Ai Phone Store Cloud',
      'login': 'Secure Login',
      'email': 'Email Address',
      'password': 'Password',
      'guest': 'Explore as Guest',
      'newPhones': 'Cloud Global Catalogs',
      'usedMarket': 'Secure Used Market',
      'selectLang': 'Select Language',
      'sellPhone': 'List Your Phone for Sale',
      'searchHint': 'Search cloud database (e.g. S26 Ultra)...',
    ...
    'ar': {
      'title': 'متجر الهواتف السحابي العالمي',
      'login': 'تسجيل الدخول الآمن',
      'email': 'البريد الإلكتروني',
      'password': 'كلمة المرور',
      'guest': 'تصفح كزائر',
      'newPhones': 'الكتالوجات السحابية',
      'usedMarket': 'سوق المستعمل الآمن',
      'selectLang': 'اختر لغة التطبيق',
      'sellPhone': 'إضافة هاتفك للبيع',
      'searchHint': 'ابحث في قاعدة البيانات السحابية...',
      'phoneName': 'اسم موديل الهاتف',
      'price': 'السعر (\$)',
      'details': 'تفاصيل الحالة',
      'submit': 'نشر الإعلان',
    },
    'es': { 'title': 'Tienda Cloud IA', 'login': 'Iniciar Sesión', 'email': 'Correo', 'password': 'Contraseña', 'guest': 'Invitado', 'newPhones': 'Cloud Catálogos', 'usedMarket': 'Usados', 'selectLang': 'Idioma', 'sellPhone': 'Vender', 'searchHint': 'Buscar...', 'phoneName': 'Modelo', 'price': 'Precio', 'details': 'Detalles', 'submit': 'Publicar' },
    'fr': { 'title': 'Boutique Cloud IA', 'login': 'Connexion', 'email': 'E-mail', 'password': 'Mot de passe', 'guest': 'Invité', 'newPhones': 'Catalogues Cloud', 'usedMarket': 'Occasion', 'selectLang': 'Langue', 'sellPhone': 'Vendre', 'searchHint': 'Rechercher...', 'phoneName': 'Modèle', 'price': 'Prix', 'details': 'Détails', 'submit': 'Publier' },
  };
              ...
              child: Column(
                ...
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.cloud_sync, size: 75, color: Colors.amber),
                  ),
...
class _MainDashboardScreenState extends State<MainDashboardScreen> {
  ...
  Widget build(BuildContext context) {
    final appState = Provider.of<AppState>(context);
    final List<Widget> pages = [const CloudCatalogsTab(), const UsedMarketTab()];
    ...
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: [
          BottomNavigationBarItem(icon: const Icon(Icons.cloud), label: appState.getText('newPhones')),
...
void showPhoneDetailsDialog(BuildContext context, Map<String, dynamic> phone) {
  showDialog(
    context: context,
    builder: (context) {
      return AlertDialog(
        backgroundColor: const Color(0xFF1E293B),
        title: Text(phone['name'] ?? 'Phone Details', style: const TextStyle(color: Colors.amber, fontWeight: FontWeight.bold)),
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  const Text('Cloud Price:', style: TextStyle(color: Colors.grey)),
                  Text(phone['price'] ?? '\$0', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.cyanAccent)),
                ],
              ),
              const Divider(height: 20, color: Colors.grey),
              const Text('Full Specifications:', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
              const SizedBox(height: 6),
              Text(phone['specs'] ?? 'No specs available', style: const TextStyle(color: Colors.grey, fontSize: 13, height: 1.4)),
              const SizedBox(height: 14),
              const Text('Cloud AI Overview:', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.amber)),
              const SizedBox(height: 4),
              Text(phone['aiVerdict'] ?? 'Synced live from Firebase Cloud.', style: const TextStyle(color: Colors.white70, fontSize: 12)),
            ],
          ),
        ),
        actions: [
...
// تبويب الجلب الحي من قاعدة بيانات سحابية (Cloud Firestore)
class CloudCatalogsTab extends StatelessWidget {
  const CloudCatalogsTab({super.key});
        ...
        children: [
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              // الاتصال بـ مجموعة 'phones' السحابية في Firebase
              ...
              builder: (context, snapshot) {
                ...
                if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                  // بيانات افتراضية تظهر في حال كانت السحابة فارغة بانتظار الإضافة
                  final List<Map<String, dynamic>> fallbackPhones = [
                    {
                      'name': 'Samsung Galaxy S26 Ultra (Cloud)',
                      'price': '\$1,399',
                      'specs': 'Snapdragon 8 Elite Gen 2 | 250MP Quad Camera | 5500mAh',
                      'aiVerdict': 'Synced via Firebase Cloud Firestore.',
                      'imageUrl': 'https://fdn2.gsmarena.com/vv/bigpic/samsung-galaxy-s25-ultra.jpg',
                    },
                    {
                      'name': 'Apple iPhone 17 Pro Max (Cloud)',
                      'price': '\$1,399',
                      'specs': 'A19 Pro Chip | 48MP Triple Periscope Lens | Titanium',
                      'aiVerdict': 'Synced via Firebase Cloud Firestore.',
                  ...
                  return ListView.builder(
                    itemCount: fallbackPhones.length,
                    itemBuilder: (context, index) {
                      final phone = fallbackPhones[index];
                      return Container(
                ...
                final filteredDocs = docs.where((doc) {
                  ...
                  return name.contains(appState.searchQuery.toLowerCase());
                }).toList();
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Container(
                      ...
                      child: ListTile(
                        ...
                        title: Text(data['name'] ?? '', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.amber, fontSize: 15)),
                        subtitle: Text(data['specs'] ?? '', style: const TextStyle(color: Colors.grey, fontSize: 11), maxLines: 2, overflow: TextOverflow.ellipsis),
                        trailing: Text(data['price'] ?? '', style: const TextStyle(color: Colors.cyanAccent, fontWeight: FontWeight.bold, fontSize: 14)),
                        onTap: () => showPhoneDetailsDialog(context, data),
                      ),
                    );
                  },
                );
              ...
              children: [
                ...
                const Row(
                  children: [
                    ...
                    SizedBox(width: 6),
                    Text('Verified Global Cloud Escrow', style: TextStyle(color: Colors.amber, fontSize: 11, fontWeight: FontWeight.bold)),
                  ],
                ),
              ],
            ),
