...
class AppState extends ChangeNotifier {
  ...
  // قاعدة بيانات الهواتف العالمية مع الدعم الكامل للغتين الإنجليزية والعربية
  final List<PhoneModel> globalPhonesDatabase = [
    PhoneModel(
      ...
      specsAr: 'معالج سناب دراجون 8 | كاميرا رباعية 250 ميجابكسل | بطارية 5500 مللي أمبير',
      aiVerdictEn: 'The absolute pinnacle of mobile technology with revolutionary AI capabilities.',
      aiVerdictAr: 'القمة المطلقة لتكنولوجيا الهواتف مع قدرات ذكاء اصطناعي ثورية.',
    ...
    PhoneModel(
      ...
      aiVerdictAr: 'أداء استثنائي وتصوير تقريب فائق الاحترافية.',
      imageUrl: 'https://fdn2.gsmarena.com/vv/bigpic/samsung-galaxy-s25-ultra.jpg',
    ),
    PhoneModel(
      ...
      aiVerdictAr: 'تحفة آبل بمعالج خارق وتصوير سينمائي فائق الدقة.',
    ...
    PhoneModel(
      ...
      aiVerdictAr: 'هاتف رائد مدمج بأداء قوي للألعاب والتصوير.',
    ...
    PhoneModel(
      ...
      specsAr: 'معالج سناب دراجون 8 جين 3 | تقريب مزدوج | عدسات هاسلبلاد',
      aiVerdictEn: 'Stunning portrait photography and supreme display brightness.',
      aiVerdictAr: 'تصوير بورتريه مذهل مع سطوع شاشة فائق.',
    ...
    PhoneModel(
      ...
      specsAr: 'ميديا تك ديمنسيتي 8200 | كاميرا 50 ميجابكسل | شحن سريع 80 واط',
      aiVerdictEn: 'Sleek design with professional portrait modes for everyday users.',
      aiVerdictAr: 'تصميم أنيق مع وضعيات بورتريه احترافية للاستخدام اليومي.',
...
class AuthWrapper extends StatefulWidget {
  ...
  @override
  State<AuthWrapper> createState() => _AuthWrapperAuthState();
}

class _AuthWrapperAuthState extends State<AuthWrapper> {
...
class GlobalCatalogsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // محرك بحث ذكي متوافق تماماً مع البحث بالعربية أو الإنجليزية (مثل "أوبو" أو "Oppo")
    List<PhoneModel> filteredPhones = appState.globalPhonesDatabase.where((phone) {
      return phone.nameEn.toLowerCase().contains(query) ||
          phone.nameAr.contains(appState.searchQuery) ||
          phone.brand.toLowerCase().contains(query) ||
          phone.specsEn.toLowerCase().contains(query) ||
          phone.specsAr.contains(appState.searchQuery);
    }).toList();

    // خاصية التوليد الذكي التلقائي في حال بحث المستخدم عن موديل جديد غير مضاف مسبقاً
    if (filteredPhones.isEmpty && appState.searchQuery.isNotEmpty) {
      filteredPhones = [
        PhoneModel(
          ...
          brand: 'Global Brand',
          price: '\$699',
          specsEn: 'Dynamic Global Specs | High-Resolution Display | Advanced Processor | Fast Charging Support',
          specsAr: 'مواصفات عالمية متطورة | شاشة عالية الدقة | معالج متقدم | دعم الشحن السريع',
          aiVerdictEn: 'Smart AI Auto-Synced Result: Verified international model specification.',
          aiVerdictAr: 'نتيجة ذكية متزامنة تلقائياً: مواصفات موديل دولي موثقة.',
          imageUrl: 'https://fdn2.gsmarena.com/vv/bigpic/samsung-galaxy-s25-ultra.jpg',
        ),
      ];
    }
