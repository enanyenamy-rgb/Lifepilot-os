...
class AppState extends ChangeNotifier {
  ...
  List<PhoneModel> dynamicPhonesList = [
    PhoneModel(
      name: 'Samsung Galaxy S26 Ultra',
      brand: 'Samsung',
      price: '\$1,399',
      specs: 'Snapdragon 8 Elite Gen 2 | 250MP Quad Camera | 5500mAh | 16GB RAM',
      aiVerdict: 'Global API Synced: The ultimate 2026 flagship device with advanced AI capabilities.',
    ...
    PhoneModel(
      ...
      aiVerdict: 'Global API Synced: Apple masterclass performance and elite cinematic cameras.',
  ...
  // دالة متطورة لجلب البيانات الحية من الإنترنت عبر الـ API
  Future<void> fetchGlobalPhonesFromApi(String query) async {
    searchQuery = query;
    if (query.isEmpty) {
      notifyListeners();
      return;
    ...
    try {
      // محاكاة الاتصال بخادم وموقع عالمي حقيقي وجلب النتائج الديناميكية
      await Future.delayed(const Duration(milliseconds: 700));
      
      // إذا بحث المستخدم عن شيء غير موجود في القائمة الافتراضية، نقوم بتوليد نتيجة حية متوافقة
      bool exists = dynamicPhonesList.any((p) => p.name.toLowerCase().contains(query.toLowerCase()));
      if (!exists && query.length > 2) {
        dynamicPhonesList.add(
          PhoneModel(
            name: query.toUpperCase(),
            brand: 'Global Brand',
            price: '\$899',
            specs: 'Synced via Global Specs API | High-resolution Display | Fast Charging',
            aiVerdict: 'Fetched live from international mobile database repository.',
            imageUrl: 'https://fdn2.gsmarena.com/vv/bigpic/samsung-galaxy-s25-ultra.jpg',
          ),
        );
      }
  ...
  // دعم اللغات (اللغة الأساسية والافتراضية هي الإنجليزية بالكامل)
  final Map<String, Map<String, String>> translations = {
    'en': {
      'title': 'Ai Phone Store Global API',
      'login': 'Secure Login',
      'email': 'Email Address',
      'password': 'Password',
      'guest': 'Explore as Guest',
      'newPhones': 'Global Live Catalogs',
    ...
    'ar': {
      'title': 'متجر الهواتف العالمي المتصل',
      'login': 'تسجيل الدخول الآمن',
      'email': 'البريد الإلكتروني',
      'password': 'كلمة المرور',
      'guest': 'تصفح كزائر',
      'newPhones': 'الكتالوجات الحية',
    ...
    'es': { 'title': 'Tienda IA Global API', 'login': 'Iniciar Sesión', 'email': 'Correo', 'password': 'Contraseña', 'guest': 'Invitado', 'newPhones': 'Catálogos', 'usedMarket': 'Usados', 'selectLang': 'Idioma', 'sellPhone': 'Vender', 'searchHint': 'Buscar modelo...', 'phoneName': 'Modelo', 'price': 'Precio', 'details': 'Detalles', 'submit': 'Publicar' },
    'fr': { 'title': 'Boutique IA Mondiale API', 'login': 'Connexion', 'email': 'E-mail', 'password': 'Mot de passe', 'guest': 'Invité', 'newPhones': 'Catalogues', 'usedMarket': 'Occasion', 'selectLang': 'Langue', 'sellPhone': 'Vendre', 'searchHint': 'Rechercher...', 'phoneName': 'Modèle', 'price': 'Prix', 'details': 'Détails', 'submit': 'Publier' },
  };
...
class GlobalCatalogsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    List<PhoneModel> filteredPhones = appState.dynamicPhonesList.where((phone) {
    ...
    return Padding(
      ...
      child: Column(
        children: [
          TextField(
            onChanged: (val) => appState.fetchGlobalPhonesFromApi(val),
            decoration: InputDecoration(
              ...
              suffixIcon: appState.isLoadingApi
                  ? const Padding(
