...
class NewPhonesScreen extends StatelessWidget {
  ...
  final List<Map<String, dynamic>> globalPhones = [
    {'name': 'iPhone 15 Pro Max', 'brand': 'Apple', 'price': '\$1199', 'specs': 'A17 Pro | 8GB RAM | 4422 mAh', 'color': Colors.blueGrey},
    {'name': 'Samsung Galaxy S24 Ultra', 'brand': 'Samsung', 'price': '\$1299', 'specs': 'Snapdragon 8 Gen 3 | 12GB RAM', 'color': Colors.indigo},
    {'name': 'Xiaomi 14 Ultra', 'brand': 'Xiaomi', 'price': '\$999', 'specs': 'Snapdragon 8 Gen 3 | Leica Camera', 'color': Colors.deepOrange},
    {'name': 'Poco X8 Pro', 'brand': 'Poco', 'price': '\$350', 'specs': 'Dimensity 8300 Ultra | 144Hz AMOLED', 'color': Colors.purple},
  ];
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    currentLang == 'ar'
                        ? '✨ اتصال سحابي عالمي نشط: عرض المواصفات والخصائص الفورية لجميع هواتف العالم.'
                        : '✨ Active Global Cloud: Instant specs for world phones.',
          ...
          const SizedBox(height: 20),
          ...globalPhones.map((phone) => _buildGlobalPhoneCard(phone['name'], phone['brand'], phone['price'], phone['specs'], phone['color'])),
        ],
      ),
    );
  }

  Widget _buildGlobalPhoneCard(String name, String brand, String price, String specs, Color cardColor) {
    return Container(
      ...
      child: Row(
        children: [
          // أيقونة هاتف متدرجة فاخرة تضمن مظهراً عالمياً مذهلاً بدون أخطاء تحميل
          Container(
            width: 75,
            height: 85,
            decoration: BoxDecoration(
              gradient: LinearGradient(colors: [cardColor, cardColor.withOpacity(0.6)], begin: Alignment.topLeft, end: Alignment.bottomRight),
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 20),
          _buildUsedCard(context, 'iPhone 14 Pro Max', 'القاهرة، مصر', '\$750', Colors.blueGrey),
          _buildUsedCard(context, 'Samsung Galaxy S23 Ultra', 'الجيزة، مصر', '\$650', Colors.indigo),
        ],
      ),
    );
  }

  Widget _buildUsedCard(BuildContext context, String title, String location, String price, Color cardColor) {
    return Container(
      margin: const EdgeInsets.symmetric(vertical: 10),
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
