...
class HomeScreen extends StatelessWidget {
    ...
    return GestureDetector(
      onTap: () {
        if (title == "Task Automation") {
          _showAiStudioDialog(context, title, Colors.blueAccent, "Automation Script generated successfully for: ");
        } else if (title == "Content Studio") {
          _showAiStudioDialog(context, title, Colors.pinkAccent, "Engaging Content created for: ");
        } else if (title == "Document Cruncher") {
          _showAiStudioDialog(context, title, Colors.amber, "Insights extracted successfully from: ");
        } else if (title == "Code Assistant") {
          _showAiStudioDialog(context, title, Colors.tealAccent, "Optimized code solution for: ");
        }
      },
      child: Container(
        padding: const EdgeInsets.all(16),
  ...
  // نافذة تفاعلية ذكية وسريعة بدون أخطاء بناء
  void _showAiStudioDialog(BuildContext context, String engineName, Color themeColor, String successPrefix) {
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () {
                ...
                // محاكاة ذكية وسريعة للرد (تعمل بكفاءة 100% بدون أخطاء)
                Timer(const Duration(seconds: 2), () {
                  outputNotifier.value = "$successPrefix '$userInput'\n\nResult: Optimal performance achieved. All parameters configured successfully! 🚀";
