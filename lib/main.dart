...
class DashboardTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(isArabic ? 'LifePilot OS 🌐' : 'LifePilot OS 🌐', style: const TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
      ),
      body: ListView(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          _buildHubCard(context, isArabic ? '🩺 القسم الطبي والتخصصي' : 'Medical Hub', isArabic ? 'إجابات طبية، استشارات صحية وأعراض فورية.' : 'Medical guidance & health advice.', Colors.red, const DynamicChatScreen(hubTitle: 'القسم الطبي والتخصصي', hintText: 'اكتب سؤالك الطبي أو الأعراض التي تشعر بها...')),
...
// شاشة المحادثة الحية المتجددة لكل الأقسام (تنبض بالحياة وتظهر أماكن جديدة للأسئلة)
...
class _DynamicChatScreenState extends State<DynamicChatScreen> {
  ...
  void _sendMessage() {
    ...
    // توليد رد ذكي بناءً على القسم
    Future.delayed(const Duration(milliseconds: 600), () {
      String aiResponse = '';
      if (widget.hubTitle.contains('الطب')) {
        aiResponse = '🩺 الاستجابة الطبية الموثوقة حول: "$userQuery"\n\n• التحليل التحليلي: بناءً على حالتك، ننصح بالراحة التامة وشرب السوائل.\n• الإرشاد: إذا استمرت الأعراض، يرجى مراجعة الطبيب المختص.\n*(تنبيه: هذه معلومة توجيهية ذكية).*';
      } else if (widget.hubTitle.contains('الديكور')) {
        aiResponse = '🏠 التصميم الهندسي المقترح بناءً على طلبك:\n\n• التخطيط: تم توزيع المساحة بذكاء لزيادة الإضاءة والاتساع.\n• الألوان والأثاث: تم اختيار ألوان متناسقة بأسلوب راقي يناسب ذوقك العصري مع إضاءة ليد مخفية.';
      } else if (widget.hubTitle.contains('الموضة')) {
        aiResponse = '👗 استشارات الموضة والأزياء الحديثة:\n\n• الصيحة الحالية: تصميمات راقية تدمج الأناقة والراحة.\n• تنسيق الألوان: درجات الألوان المتناسقة المقترحة تمنحك إطلالة استثنائية وجذابة لكل الفئات.';
      } else {
        aiResponse = '💡 إجابة ذكية ومفصلة حول: "$userQuery"\n\nتم استعراض قاعدة المعارف العالمية لتقديم إجابة شاملة ودقيقة تلبي تطلعاتك بالكامل.';
      }
                ...
                ? Center(
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          const Icon(Icons.chat_bubble_outline, size: 60, color: Colors.indigo),
      ...
      body: ListView(
        ...
        children: [
          ...
          const ListTile(
            ...
            subtitle: Text('LifePilot OS - Live Interactive v3.0'),
          ),
        ],
      ),
    );
